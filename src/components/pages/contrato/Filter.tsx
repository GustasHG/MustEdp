...
import { getContratoPath } from "./ContratoPath";
...
interface FilterContratoProps {
}

export default function FilterContrato(
    {
    ...
    const changeRegiaoEventHandler = (value: string) => {
        window.location.replace(
            getContratoPath(value, params.get("ponto"), params.get("posto"), params.get("ano"))
        );
    }

    const changePontoEventHandler = (value: string) => {
        window.location.replace(
            getContratoPath(params.get("regiao"), value, params.get("posto"), params.get("ano"))
        );
    }

    const changePostoEventHandler = (value: string) => {
        window.location.replace(
            getContratoPath(params.get("regiao"), params.get("ponto"), value, params.get("ano"))
        );
    }
    const changeAnoEventHandler = (value: string) => {
        window.location.replace(
            getContratoPath(params.get("regiao"), params.get("ponto"), params.get("posto"), value)
        );
    }
